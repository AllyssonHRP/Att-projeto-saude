document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!pacienteId) {
        window.location.href = '/Projeto_Saude/Projeto-Saude front/login page/login.html';
        return;
    }

    document.getElementById('setup-mfa-button').addEventListener('click', async () => {
        try {
            const response = await fetch(`http://localhost:5500/api/auth/mfa/setup?pacienteId=${pacienteId}`, {
                method: 'POST'
            });
    ...
    document.getElementById('verify-mfa-button').addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5500/api/auth/mfa/verify?pacienteId=${pacienteId}&code=${code}`, {
                method: 'POST'
            });
            if (response.ok) {
